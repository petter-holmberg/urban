...
inline constexpr auto MOVE_DELAY = 5;
...
/**************************************************************************/
soldier3_o::soldier3_o(int X, int Y, int Z)
    : Object(X, Y, Z)
{
    PALETTE pal;
    char filename[512];
...
auto soldier3_o::update() -> int
{
    ...
    if ((state != STATE_FIRE && state != STATE_FIRING && state != STATE_TURN && state != STATE_ALARM && state != STATE_HIT && state != STATE_HIT_COUNTDOWN)) {
        if (engine->player->GetX() < x && engine->player->GetX() > x - FIRE_RANGE && direction == LEFT_DIR && engine->player->GetLayer() == layer) {
            state = STATE_FIRE;
        } else if (engine->player->GetX() > x && engine->player->GetX() < x + FIRE_RANGE && direction == RIGHT_DIR && engine->player->GetLayer() == layer) {
            state = STATE_FIRE;
        }
    }
