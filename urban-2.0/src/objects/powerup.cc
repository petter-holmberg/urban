...
void powerup_o::Collision(Object* o)
{
    ...
    if (o->GetWho() == FRIEND_PLAYER) {
        ...
        switch (type) {
        case POWERUP_EXTRA_LIFE:
            (dynamic_cast<player_o*>(engine->player))->AddLife();
            break;
        case POWERUP_WEAPON_FLAME_THROWER:
            if ((dynamic_cast<player_o*>(engine->player))->HaveWeapon(FLAME_THROWER) != 0) {
                energy = 1;
                break;
            }
            (dynamic_cast<player_o*>(engine->player))->EnableWeapon(FLAME_THROWER);
            break;
        case POWERUP_WEAPON_ICEMAKER:
            if ((dynamic_cast<player_o*>(engine->player))->HaveWeapon(ICEMAKER) != 0) {
                energy = 1;
                break;
            }
            (dynamic_cast<player_o*>(engine->player))->EnableWeapon(ICEMAKER);
            break;

        case POWERUP_WEAPON_GRENADE_LAUNCHER:
            if ((dynamic_cast<player_o*>(engine->player))->HaveWeapon(GRENADE_LAUNCHER) != 0) {
                energy = 1;
                break;
            }
            (dynamic_cast<player_o*>(engine->player))->EnableWeapon(GRENADE_LAUNCHER);
            break;

        case POWERUP_WEAPON_PLASMA:
            if ((dynamic_cast<player_o*>(engine->player))->HaveWeapon(PLASMA_GUN) != 0) {
                energy = 1;
                break;
            }
            (dynamic_cast<player_o*>(engine->player))->EnableWeapon(PLASMA_GUN);
            break;

        case POWERUP_WEAPON_ELECTRIC:
            if ((dynamic_cast<player_o*>(engine->player))->HaveWeapon(ELECTRIC) != 0) {
                energy = 1;
                break;
            }
            (dynamic_cast<player_o*>(engine->player))->EnableWeapon(ELECTRIC);
            break;

        case POWERUP_WEAPON_MINIGUN:
            if ((dynamic_cast<player_o*>(engine->player))->HaveWeapon(MINIGUN) != 0) {
                energy = 1;
                break;
            }
            (dynamic_cast<player_o*>(engine->player))->EnableWeapon(MINIGUN);
            break;

        case POWERUP_AMMO_FLAME_THROWER:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(FLAME_THROWER, 50);
            break;

        case POWERUP_AMMO_ICEMAKER:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(ICEMAKER, 50);
            break;

        case POWERUP_AMMO_GRENADE_LAUNCHER:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(GRENADE_LAUNCHER, 1);
            break;

        case POWERUP_AMMO_GRENADE_LAUNCHER6:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(GRENADE_LAUNCHER, 6);
            break;

        case POWERUP_AMMO_PLASMA:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(PLASMA_GUN, 20);
            break;

        case POWERUP_AMMO_MINIGUN:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(MINIGUN, 100);
            break;

        case POWERUP_AMMO_ELECTRIC:

            (dynamic_cast<player_o*>(engine->player))->AddAmmo(ELECTRIC, 20);
            break;
