...
auto Engine::display_map() -> int
{
    ...
    for (int i = bgstartx; i < (bgstartx + 4); i++) {
        for (int j = (bgstarty + 3); j >= bgstarty; j--) {
            if (j < 0) { //<- OBS!!! HACK
                continue;
            }
    ...
    for (int k = 0; k < 3; k++) {
        ...
        draw_player = 0;
        for (int i = startx; i < (startx + 14); i++) {
            for (int j = (starty + 10); j >= starty; j--) {
        ...
        for (int i = 0; i < num_innerlayer; i++) {
        ...
        for (int i = 0; i < num_pre_effect; i++) {
        ...
        for (int i = 0; i < (ringbuffer != 0 ? MAX_DEKORATION_OBJECTS : num_dekorations); i++) {
        ...
        for (int i = 0; i < num_alwaysupdate; i++) {
    ...
    for (long unsigned int i = 0; i < NUM_LETTERS; i++) {
        if ((key[static_cast<size_t>(Letters[i].key)]) && cheat_cl_p < 63) {
            ...
            int clear_cc = 1;
            for (long unsigned int j = 0; j < NUM_CHEATCODES; j++) {
