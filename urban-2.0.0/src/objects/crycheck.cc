...
/**************************************************************************/
Crycheck_o::Crycheck_o(int X, int Y, int Z) : Object(X, Y, Z) {
	RGB pal[256];
        char filename[512];
        int i;
...
int Crycheck_o::update() {
	...
	if (!counter && state != STATE_COLLAPS && state != STATE_WAITING && state != STATE_DYING && state != STATE_FIRING) {
        ...
        if ((state != STATE_FIRE && state != STATE_FIRING && state != STATE_TURN &&
        	state != STATE_COLLAPS && state != STATE_DYING && state != STATE_WAITING))
					...
					case 1:
						...
						r = anim.next_frame(2, 10);
								...
								ENGINE.create_effect(new shells_o(x + 23, y + 22, z, "soldier/hylsa", 1, random() % 5, -(random() % 8), 0));
                                                	...
                                                	counter2 = 1;
							if(direction == LEFT_DIR && PLAYER->GetX() < x && PLAYER->GetX() > x - FIRE_RANGE &&
                	                                	PLAYER->GetLayer() == layer)
                                                                r = 0;
							else if (direction == RIGHT_DIR && PLAYER->GetX() > x && PLAYER->GetX() < x + FIRE_RANGE &&
	                                             		PLAYER->GetLayer() == layer)
                                                                r = 0;
						}
...
/*                        	current_image = (direction == RIGHT_DIR ? 22 : 20) + (r = anim.next_frame(1, FIRING_DELAY));
	...
	layer = z / TILE_TOP_HEIGHT;
        ...
        if (!speed_x && !speed_z && state != STATE_FIRING && state != STATE_COLLAPS &&
        	state != STATE_WAITING && state != STATE_DYING) {
...
/**************************************************************************/
Boss_Crycheck_o::Boss_Crycheck_o(int X, int Y, int Z) : Crycheck_o(X, Y, Z) {
	score = 1500;
        energy = 30;
}
