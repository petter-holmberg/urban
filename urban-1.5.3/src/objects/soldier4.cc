...
#define FIRE_RANGE		250
#define FIRE_STRENGTH		5
#define INTERLAYER_FIRE_RANGE	150
...
/**************************************************************************/
soldier4_o::soldier4_o(int X, int Y, int Z) : Object(X, Y, Z) {
	RGB pal[256];
        char filename[512];
        int i;
...
//                        	&& PLAYER->GetLayer() == layer) {
				if (PLAYER->GetLayer() == layer || (
                                	PLAYER->GetX() < x - INTERLAYER_FIRE_RANGE))
	                                state = STATE_FIRE;
